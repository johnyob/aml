...
module type S = sig
  ...
  module Type : sig
    ...
    (** [unsafe_link ~src ~dst] equates [src] to [dst], but ignores [src]'s structure. *)
    val unsafe_link : src:t -> dst:t -> unit
  end
...
module Make (S : Structure.Basic) () = struct
  ...
  module Type = struct
    ...
    let rec sexp_of_t t = [%sexp_of: t S.t] (structure t)

    let unsafe_link ~src ~dst =
      let desc = Union_find.get dst in
      Union_find.union src dst;
      Union_find.set src desc
    ;;
  end
  ...
  module Make_unify (M : Structure.Merge with type 'a t := 'a S.t) = struct
    type unify_fn = ctx:Type.t M.ctx -> Type.t -> Type.t -> unit
    ...
    module Work_queue : sig
      ...
      val create : unit -> t
      val enqueue : t -> unify_fn
      val run : t -> f:unify_fn -> unit
    end = struct
      (* A stack is used since it will likely yield errors earlier. *)
      type t = (Type.t M.ctx * Type.t * Type.t) Stack.t

      let create () = Stack.create ()
      let enqueue t ~ctx type1 type2 = Stack.push t (ctx, type1, type2)

      let run t ~f =
        let rec loop () =
          match Stack.pop t with
          | None -> ()
          | Some (ctx, type1, type2) ->
            f ~ctx type1 type2;
            loop ()
        in
        loop ()
    ...
    let rec unify_exn ~ctx ~work_queue type1 type2 =
      ...
      Work_queue.run work_queue ~f:(unify_exn ~work_queue)
    ;;
