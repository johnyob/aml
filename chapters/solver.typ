...
#syntax(
  syntax-rule(
    name: [Frames], 
    $F ::= &square and C |  exists alpha. square | forall a. square | exists scopev. square \ 
    | &cdef x : sigma cin square \
    | &clet x : cforall(Theta, square, tau)  cin C \
...
#syntax(
  ...
  syntax-rule(name: [Unification Problems], $U ::=& ctrue | cfalse | U and U | exists alpha. U | exists scopev. U \ | &umultieq_Ty | umultieq_Scope $), 

  syntax-rule(name: [Multi-equations (Types)], $umultieq_Ty ::= dot | umultieq_Ty = tau$),

  syntax-rule(name: [Multi-equations (Scopes)], $umultieq_Scope ::= dot | umultieq_Scope = Psi$)
)
