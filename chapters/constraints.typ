...
$
  ...
  phi_Ty ([Psi] tau) &= [phi_Scope (Psi)] phi_Ty (tau) &#h(2cm)&& phi_Scope (Psi, eqname) &= phi_Scope (Psi) and phi(eqname) 
$
...
$
  ...
  #proof-tree(
    rule(
      ...
      $eqname disjoint phi$, 
      label: [$kappa' = (floor(phi(tau_1)) = floor(phi(tau_2)))$],
      name: [(Impl)]
    )
  )
...
// Implications 
#comment[TODO: Explain implications]
...
We cannot _erase_ scopes like we do in the #aml type system, however, we can define a constraint $ceil(tau) = alpha$ which states that $alpha$ is equal to $tau$ with some scopes _inserted_. 
$
