...
#syntax(
  ...
  syntax-rule(name: [Types], $tau^kappa in Ty ::= alpha^kappa | overline(tau^ty) tformer | [tau^scope] tau | emptyset^scope | tau^scope, eqname$),
  syntax-rule(name: [Type Schemes], $sigma in Scm ::= tau | tforall(alpha :: kappa) sigma $),
  syntax-rule(name: [Kinds], $kappa in Kind ::= ty | scope$),
  syntax-rule(
    name: [Contexts],
    $Gamma in Ctx ::= &emptyset | Gamma, alpha :: kappa | Gamma, eqname : tau = tau | Gamma, x: sigma$,
  ),
)
...
#judgement-box($Gamma tack tau^kappa :: kappa$, $Gamma tack sigma scm$, $Gamma ctx$)

$

  #proof-tree(
    rule(
      $Gamma tack alpha :: kappa$,
  ...
  #proof-tree(
    rule(
      $Gamma tack overline(tau) tformer :: ty$,
      $forall i, space Gamma tack tau_i :: ty$, 
      name: [(CstrWF)]
    )
  )
  ...
  #proof-tree(
    rule(
      ...
      $Gamma, alpha :: kappa tack sigma scm$,
...
// Equivalence 
_Equivalence and Equality._ Types have an associated equivalence relation, written $Gamma tack tau_1 equiv tau_2 space [Psi]$, read: _under the context $Gamma$, $tau_1$ and $tau_2$ are equivalent in the scope $Psi$_. We refer to $Psi$ as the _support_ in the equivalence relation. We often write $Gamma tack tau_1 equiv tau_2$ if the support is empty. 

To prove equivalences, we can rely on equalities referenced in $Psi$ as well as standard rules such as reflexivity, symmetry, transitivity, congruence, and decomposition. Type constructors are injective.  
Additionally, we introduce three rules associated with scoped ambivalent types. 

// equiv Amb
The first rule permits us to prove equivalences between scoped types. We may only use this rule when the support is _empty_. This means that types with nested scopes may only use their top-level scope as a support. We impose this restriction 
to avoid handling scope concatenation in our constraint solver. Allowing nested supports would be equivalent to introducing the rule: $[Psi_1][Psi_2]tau equiv [Psi_1, Psi_2]tau$ (Axiom S4). While this rule is entirely sound, it complicates the constraint solver by requiring it to handle row concatenation -- a historically challenging problem in type inference. 
...
These rules are inspired by _contextual modal types_. Namely we can interpret $[Psi] tau$ as a contextual modal type $square_Psi tau$.
With this view, all of these rules naturally arise in modal logic, a connection that we formalize in our chapter on #xaml, the _Explicit #aml _ calculus.
...
$
  ...
  #proof-tree(
    rule(
      $Gamma tack [Psi]tau_1 equiv [Psi]tau_2 space [emptyset]$, 
      $Gamma tack tau_1 equiv tau_2 space [Psi]$, 
      name: [($equiv$Amb)]
    )
  )
  ...
  #proof-tree(
    rule(
      $Gamma tack [Psi](overline(tau) tformer) equiv overline([Psi]tau) tformer space [Psi']$, 
      $arity(tformer) > 0$, 
      name: [($equiv$Dist)]
    )
  )
...
$
  ...
  #proof-tree(
    rule(
      ...
      $Gamma, alpha :: kappa tack e : sigma$,
      $alpha disjoint Gamma$,
...
$
  ...
  #proof-tree(
    rule(
      ...
      $Gamma, alpha :: kappa tack sigma <= sigma'$,
      $alpha disjoint Gamma$
    )
  )
$
...
Pattern matching on equalities using $ematch e_1 : tau_1 = tau_2 ewith erefl -> e_2$ can eliminate the equality witness $e_1$ of type $tau_1 = tau_2$, adding it as an implicit equality to the context $Gamma$ while type checking the body $e_2$; the witness $e_1$ must have the structure of $tau_1 = tau_2$ but may have additional scopes -- we formalize this using a _scope insertion_ function. Since the equation is only available in the scope of $e_2$, it must not be present in the return type $sigma$. We ensure this by allocating a fresh name for the equality $eqname disjoint Gamma$ and ensuring $eqname$ doesn't occur in $sigma$, this is done so by using the well-formedness judgement $Gamma tack sigma scm$.
If the return type doesn't satisfy this condition, then we say the _equation ($eqname$) has escaped its scope_.

_Annotations_. Annotations represent an explicit loss of sharing of scopes between types. This loss of sharing of scopes permits us to eliminate ambivalence in our return type, thereby preventing scope escapes. The types $tau_1, tau_2$ need not be identical to $tau$ but must instead be _consistent instances_ of $tau$. We define what it means to be a _consistent instance_ below.

// Rigid instances definition
We begin by defining a _scope insertion_ function $ceil(tau)$ for types, which produces a context of scope variables $cal(S)$ and a type $tau'$ with scopes inserted at _leaves_ of the type. Scopes at inner nodes can be inferred via the distributivity of scopes.
...
$
  ...
  ceil([Psi]tau) &= ceilret(scopev, [scopev][Psi]tau) && "fresh" scopev\
  ceil(overline(tau) tformer) &= ceilret(#[$cal(S)_1, ..., cal(S)_n$], overline(tau')) tformer &#h(2cm)&"where" ceil(tau_i) = ceilret(cal(S)_i, tau'_i)
$

We write $forall ceil(tau)$ for $tforall(cal(S)) tau'$ where $ceil(tau) = ceilret(cal(S), tau')$. This type scheme describes a set of instances that are variants of $tau$ with differing scopes. We say such instances are _consistent instances_ of $tau$. 
...
in the presence of scopes, if $e_1$ have the type $[Psi]tau$ where $tau$ and $Gamma tack tau equiv tau_1 -> tau_2 space [Psi]$,
...
_Let Bindings and Generalization_. Let bindings $elet x = e_1 ein e_2$ assign a polymorphic type $sigma_1$ to $x$ in the scope of $e_2$. 
Generalization is mostly standard with the additional requirement that the generalizable variable $alpha$ cannot occur in 'dangerous' positions in the type scheme. A dangerous position is one that is under a (non-trivial #footnote([All trivial scopes may be removed using equivalence])) scope. We formally define this as follows:
$
...
Without this condition, a loss of principality can occur. We demonstrate this by first considering the type of `coerce`, defined follows:
```ocaml
let coerce = fun (type a b) w x ->
  ...
  (x : b)
```
Without the restriction on generalizable variables, multiple types could be inferred for `coerce`, neither of which is more general than the other:
$
...
$
It is interesting to note that the type is not more general by permitting a scope on the equality since $[scopev_5]([scopev_1]alpha = [scopev_2]beta) equiv [scopev_5, scopev_1]alpha = [scopev_5, scopev_2]beta$. 

Moreover, allowing the latter type schemes leads to some morally ambivalent programs being well-typed in our system. For example:
```ocaml
let should_not_typecheck (type a) (w : (a, int) eq) (x : a) =
  ...
  coerce Refl y
```
We note that `y` has the type $[eqname : alpha = tint]alpha$ (or $[eqname : alpha = tint]tint$), yet we can infer $tint$ or $alpha$ for the result of `should_not_typecheck`. This also leads to a loss in principality. 

To prevent such issues, our notion of 'dangerous' variables enforces that in order to generalize $alpha, beta$, the type system must ensure they do not occur under scopes. With this restriction, the principal type for `coerce` is:
$
  forall alpha, beta. space (alpha = beta) -> alpha -> beta
$
